...
async def test_dry_run_covers_all_scenes(tmp_path):
    ...
    assert "вопрос закрыт" in md
    assert "✅ Родитель подтвердил: всё в порядке" in md

    # 2b/2c: координатор узнал об опоздании (кнопка + свободный текст).
    # R9-14: оба пути теперь содержат интервал «на 15 мин».
    assert md.count("⏰ Родитель сообщил: ученик опоздает (на 15 мин)") == 2
    assert md.count("На сколько минут ученик опоздает?") >= 1
    assert "💬 «Мы опоздаем на 15 минут — пробки»" in md  # исходный текст дошёл цитатой
