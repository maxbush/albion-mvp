...
- [ ] **P0.4** Markdown без `parse_mode` — пользователь видит служебные символы:
...
- [ ] **P1.1** Интент `absence_report` классифицируется AI-слоем, но **не имеет обработчика**:
...
- [ ] **P1.2** Протухшие check-in workflow'ы: `find_active_checkin` подхватывает ЛЮБОЙ
...
- [ ] **P2.2** `MockMeritHubService.get_balance / check_low_balance` — не используются
  ничем, кроме собственного теста (`PAYMENT_*` события нигде не публикуются — реального
  источника баланса нет). Удалить методы + тест-assert'ы (или перенести в backlog-блок).
...
1. **H1 — Самоназначение роли coordinator (privilege escalation).** По кнопке
   ...
   получать эскалации, лиды и алерты (PII учеников). Это осознанная фича демо-пилота?
   Варианты: (a) оставить для демо; (b) самоназначение coordinator только если TG ∈
   `ALBION_ADMIN_TELEGRAM_IDS`, иначе роль parent + заявка админу (/role). **По умолчанию
   не меняю** — жду решения.
