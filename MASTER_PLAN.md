...
## 🔍 САМОАУДИТ ПОСЛЕ ROUND 10 (2026-08-02)

- [x] **SA-1. «Опоздаю» в pre-lesson не отменял `*_no_reply`** — координатор получил бы
  ложное «Родитель/репетитор не ответил» в добавок к «опоздает» (баг внесён в R10-П3,
  найден при самопроверке). Фикс: `cancel_by_workflow` перед созданием fallback.
  Новый тест-регрессия.
- [x] **SA-2. `notify_late_detail` мог отправить дубль после сработавшего fallback** —
  guard по `state == "running"`.
- [x] **SA-3. Чистка строк П9** (убрана текстовая «кнопка» из списка строк алерта).
- [x] **SA-4. Проверки**: /incidents с review (статистика + кнопка закрытия), demo_dry_run
  (полная расшифровка), два подряд прогона pytest без флейков, pyflakes чист.
  Итог: **239/239 тестов**.

---

## 🔍 САМОАУДИТ PASS 2 (2026-08-02)

- [x] **SA-5. Kill switch: разные подписи в /kill_switch и /status** — унифицированы
  («Всё остановлено / Только алерты координаторам / Всё работает»).
- [x] **SA-6. «Отмена: Миша — Миша — математика»** — дубль имени, когда title класса
  уже содержит имена; фикс через `subject_full` (airtable-предмет — с префиксом).
- [x] **SA-7. Чужие аккаунты не могут жать кнопки** — checkin/resolve/resolve_late_time
  проверяют `actor_telegram_id`/`parent_telegram_id` (пересланные кнопки отклоняются).
- [x] **SA-8. Сквозные проверки**: полный цикл П1 (no_show → отмена координатором:
  ровно 2/2/1 сообщения, ноль висящих задач), review → «Разобрался», demo_dry_run
  (5 сцен), julianday парсит ISO+offset, git не трекает .db/.log/.venv.
  Итог: **241/241 тестов**.
