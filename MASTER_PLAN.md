...
- [x] **U4** Онбординг: role-specific expectations (без обещаний несуществующих
  механик); `/start` координатора — 1 сообщение + кнопка «📋 Команды» (help по
  требованию, с возвратом «Назад»).
- [x] **U5** Тексты: убрать дубль-список в absence-уведомлении; подтверждение с
  именем ученика; «Ошибка: некорректные данные.» → человеческая фраза.
- [x] **U6** Интент «отмена» → inline-кнопки с реальными занятиями (из
  merithub_classes) вместо ручного ввода ID; fallback — текстовая подсказка.
- [x] **U7** Первый текст от незнакомца → выбор роли вместо тихого auto-parent.

## Журнал Round 4

- ✅ U1 меню команд по роли → `5bebc78` (4 теста)
- ✅ U2 кнопки эскалации (coord_resolve + tg://user url) → `8dcee54` (4 теста)
- ✅ U3 demo_reset confirm + kill_switch кнопки → `dd3655c` (5 тестов)
- ✅ U4 честный онбординг + help по кнопке → `71bc2b3` (5 тестов)
- ✅ U5 чистка текстов уведомлений → `f2fd496` (2 теста)
- ✅ U6 отмена кнопками со списком занятий → `79bb1d2` (3 теста)
- ✅ U7 первый текст → выбор роли → `75307a9` (3 теста)
- Итог: **7/7 UX-задач**, 26 новых тестов, **153/153 passing**, 9 коммитов.
