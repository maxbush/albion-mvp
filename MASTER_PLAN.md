...
## Round 5 (2026-07-31) — пакет для клиента: демо-прогон, инструкция владельца

- [x] **D1** Сухой прогон демо: `scripts/demo_dry_run.py` — детерминированный
  прогон сцен из DEMO_RUNBOOK (2a–2e + инциденты) на реальных workflow,
  без Telegram/сети; печатает расшифровку, `--out` сохраняет в файл.
- [x] **D2** `DEMO_TRANSCRIPT.md` — авто-расшифровка, регенерируется перед
  каждым демо; smoke-тест `tests/test_demo_dry_run.py` не даёт сценарию сгнить.
- [x] **D3** Баг, пойманный сухим прогоном: `/cancel_lesson <unknown>` отвечал
  и «❌ не найден», и «🔄 передана координаторам». Фикс: урок проверяется ДО
  публикации события; workflow-фидбэк оставлен защитой от гонки. TDD: сначала
  красный тест, потом фикс → `c7324ed`.
- [x] **D4** `OWNER_GUIDE.md` — инструкция для владельцев (текст под
  презентацию/брошюру: 18 разделов ≈ слайдов, только реализованные фичи).
- [x] README: бейдж 155, раздел «Документация по аудиториям»; DEMO_RUNBOOK
  ссылается на сухой прогон.

### Журнал Round 5

- ✅ фикс /cancel_lesson → `c7324ed` (2 теста: новый контракт + защита от гонки)
- ✅ dry-run скрипт + расшифровка + smoke-тест → `d4c3f0a` (1 тест; engine
  нацеливается в БД прогона — conftest-совместимость)
- Итог: **155/155 passing**, +3 теста, пакет для клиента собран.
